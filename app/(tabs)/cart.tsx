...
const PaymentInfoStripe = ({ label, value, labelStyle, valueStyle }: PaymentInfoStripeProps) => (
	<View className='flex-between flex-row my-1'>
		<Text className={clsx('paragraph-medium text-gray-200', labelStyle)}>{label}</Text>
		<Text className={clsx('paragraph-bold text-dark-100', valueStyle)}>{value}</Text>
	</View>
);
	...
	return (
		<SafeAreaView className='bg-white h-full'>
			<FlatList
				...
				ListEmptyComponent={() => <Text className='text-center text-dark-100'>Your cart is empty</Text>}
				ListFooterComponent={() =>
					totalItems > 0 && (
